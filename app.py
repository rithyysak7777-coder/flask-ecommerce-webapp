from flask import Flask, render_template, request, make_response, redirect, url_for, jsonify, session, flash, request
...
class User(db.Model):
    ...
    profile = db.Column(db.String(255), nullable=False, default='/static/image/default.png')
...
def user():
    ...
    # row = result.fetchall() // lists of rows
    rows = [dict(row._mapping)for row in result] # result,loop and store in rows of dictionary
    # assert False;
...
def do_add_user():
    ...
    u = User(
        ...
        password=password,
        role=role,
        profile='/static/image/default.png',
        status='Active'
    )
    db.session.add(u)
    db.session.commit()
...
def conform_delete(user_id):
    ...
    if result:
        ...
        return redirect(url_for('user'))
    return render_template('admin/user/comfirm_delete.html', module = module , user = user)  #see and get it out to use
